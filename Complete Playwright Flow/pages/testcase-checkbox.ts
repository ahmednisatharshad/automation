...
export class CheckboxLocators {
  ...
  // ===== Newly added locators for integration navigation =====
  elementsMenu = () =>
    this.page.locator('span').filter({ hasText: 'Elements' }).locator('div').first();

  widgetsMenu = () =>
    this.page.locator('span').filter({ hasText: 'Widgets' }).locator('div').first();

  autoCompleteMenuItem = () =>
    this.page.getByRole('listitem').filter({ hasText: 'Auto Complete' });
}
